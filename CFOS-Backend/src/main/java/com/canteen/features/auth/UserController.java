...
public class UserController {
    ...
    @PostMapping
    public java.util.concurrent.CompletableFuture<ResponseEntity<CreateUserResModel>> createUser(
            @Valid @RequestBody CreateUserReqModel request,
            HttpServletRequest httpRequest) {
        String creatorRole = (String) httpRequest.getAttribute("role");
        return java.util.concurrent.CompletableFuture.supplyAsync(() -> {
            try {
                CreateUserResModel response = userService.createUser(request, creatorRole);
